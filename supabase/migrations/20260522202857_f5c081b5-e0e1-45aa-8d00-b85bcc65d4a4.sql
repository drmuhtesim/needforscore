CREATE OR REPLACE FUNCTION public.get_entries_feed(_category text DEFAULT 'all'::text, _search text DEFAULT ''::text, _limit integer DEFAULT 100)
 RETURNS TABLE(id uuid, user_id uuid, target text, target_normalized text, category entry_category, description text, rating smallint, verified_target boolean, created_at timestamp with time zone, deleted_at timestamp with time zone, deleted_by uuid, profile_user_id uuid, profile_username text, profile_display_name text, profile_avatar_url text, profile_city text, profile_occupation text, profile_age smallint, profile_bio text, profile_show_avatar boolean, profile_show_display_name boolean, profile_show_city boolean, profile_show_occupation boolean, profile_show_age boolean, profile_show_bio boolean, profile_show_linked_accounts boolean, profile_signup_order bigint, vote_score integer, comment_count integer, avg_rating numeric, my_vote integer, last_comment_excerpt text)
 LANGUAGE sql
 STABLE
 SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.get_entries_feed(text, text, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_entries_feed(text, text, integer) TO anon, authenticated;